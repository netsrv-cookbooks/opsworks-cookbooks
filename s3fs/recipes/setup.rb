...
unless platform?('amazon')
  fail 'This recipe can only be used on Amazon Linux instances'
end

%w(gcc libstdc++-devel gcc-c++ curl-devel libxml2-devel openssl-devel mailcap fuse fuse-devel).each do |pkg|
  package pkg do
    action :install
  end
end
